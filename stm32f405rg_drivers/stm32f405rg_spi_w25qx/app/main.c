...
int main(void)
{
    ...
    uart_debug.ops->printf(&uart_debug, "=== W25QX Function Test Start ===\r\n");
    uart_debug.ops->printf(&uart_debug, "1. Initialization test: ");
    if (ret != 0) {
        uart_debug.ops->printf(&uart_debug, "FAILED (ret=%d)\r\n", ret);
        goto test_end;
    }
    uart_debug.ops->printf(&uart_debug, "OK\r\n");

    /* 2. 读取ID测试 */
    uart_debug.ops->printf(&uart_debug, "2. Read ID test: ");
    ret = w25q128.ops->read_id(&w25q128, &mid, &did);
    if (ret != 0) {
        uart_debug.ops->printf(&uart_debug, "FAILED (ret=%d)\r\n", ret);
        goto test_end;
    }
    uart_debug.ops->printf(&uart_debug, "OK (mid:0x%02X, did:0x%04X)\r\n", mid, did);

    /* 3. 唤醒测试 */
    uart_debug.ops->printf(&uart_debug, "3. Wakeup test: ");
    ...
    /* 4. 64KB块擦除测试 */
    uart_debug.ops->printf(&uart_debug, "4. 64KB Block Erase test: ");
    ...
    for (i = 0; i < W25QX_PAGE_SIZE; i++) {
        if (read_buf[i] != 0xFF) {
            uart_debug.ops->printf(&uart_debug, "FAILED (erase incomplete)\r\n");
            goto test_end;
        }
    }
    uart_debug.ops->printf(&uart_debug, "OK\r\n");
    ...
    /* 5. 页写入测试 */
    uart_debug.ops->printf(&uart_debug, "5. Page Write test: ");
    for (i = 0; i < W25QX_PAGE_SIZE; i++) {
    ...
    if (error_cnt != 0) {
        uart_debug.ops->printf(&uart_debug, "FAILED (errors=%d)\r\n", error_cnt);
        goto test_end;
    }
    uart_debug.ops->printf(&uart_debug, "OK\r\n");

    /* 6. 4KB扇区擦除测试 */
    uart_debug.ops->printf(&uart_debug, "6. 4KB Sector Erase test: ");
    ...
    /* 7. 跨页写入测试 */
    uart_debug.ops->printf(&uart_debug, "7. Cross-page Write test: ");
    ...
    /* 8. 去初始化测试 */
    uart_debug.ops->printf(&uart_debug, "8. Deinitialization test: ");
...
test_end:
    uart_debug.ops->printf(&uart_debug, "=== W25QX Function Test Complete ===\r\n\r\n");
    while (1) {
