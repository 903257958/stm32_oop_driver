...
int main(void)
{
    ...
    // 1. 擦除测试
    uart_debug.ops->printf(&uart_debug, "1. Erase test: ");
    #if DRV_FLASH_PLATFORM_STM32F1 || DRV_FLASH_PLATFORM_GD32F1
    ...
    #endif
    if (ret != 0) {
        uart_debug.ops->printf(&uart_debug, "FAILED (erase ret=%d)\r\n", ret);
        goto end;
    }
    // 验证擦除（应为0xFFFFFFFF）
    for (i = 0; i < 16; i++) {
    ...
    if (error_cnt != 0) {
        uart_debug.ops->printf(&uart_debug, "FAILED (erase verify err=%d)\r\n", error_cnt);
        goto end;
    }
    uart_debug.ops->printf(&uart_debug, "OK\r\n");

    // 2. 写入测试
    uart_debug.ops->printf(&uart_debug, "2. Write test: ");
    ret = flash.ops->write(&flash, test_addr, 64, write_buf);
    if (ret != 0) {
        uart_debug.ops->printf(&uart_debug, "FAILED (write ret=%d)\r\n", ret);
        goto end;
    }
    uart_debug.ops->printf(&uart_debug, "OK\r\n");

    // 3. 读取验证
    uart_debug.ops->printf(&uart_debug, "3. Read verify: ");
    error_cnt = 0;
    for (i = 0; i < 16; i++) {
    ...
    if (error_cnt != 0) {
        uart_debug.ops->printf(&uart_debug, "FAILED (mismatch=%d)\r\n", error_cnt);
        goto end;
    }
    uart_debug.ops->printf(&uart_debug, "OK\r\n");

end:
    uart_debug.ops->printf(&uart_debug, "=== Test Complete ===\r\n\r\n");
    while (1);
}
