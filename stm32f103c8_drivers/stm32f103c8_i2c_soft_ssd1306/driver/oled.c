...
/**
  ...
  */
uint8_t g_oled_diaplay_buf[MAX_OLED_NUM][8][128];			
...
/* 通信协议 */
static int8_t __oled_write_command(oled_dev_t *dev, uint8_t command);
static int8_t __oled_write_data(oled_dev_t *dev, uint8_t *data, uint8_t count);

/* 功能函数 */
static int8_t __oled_update(oled_dev_t *dev);
static int8_t __oled_update_area(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height);
static int8_t __oled_clear(oled_dev_t *dev);
static int8_t __oled_clear_area(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height);
static int8_t __oled_reverse(oled_dev_t *dev);
static int8_t __oled_reverse_area(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height);
static int8_t __oled_show_image(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height, const uint8_t *image);
static int8_t __oled_show_char(oled_dev_t *dev, uint8_t x, uint8_t y, char chr, uint8_t size);
static int8_t __oled_show_string(oled_dev_t *dev, uint8_t x, uint8_t y, char *string, uint8_t size);
static int8_t __oled_show_num(oled_dev_t *dev, uint8_t x, uint8_t y, uint32_t num, uint8_t len, uint8_t size);
static int8_t __oled_show_signed_num(oled_dev_t *dev, uint8_t x, uint8_t y, int32_t num, uint8_t len, uint8_t size);
static int8_t __oled_show_hex_num(oled_dev_t *dev, uint8_t x, uint8_t y, uint32_t num, uint8_t len, uint8_t size);
static int8_t __oled_show_bin_num(oled_dev_t *dev, uint8_t x, uint8_t y, uint32_t num, uint8_t len, uint8_t size);
static int8_t __oled_show_float_num(oled_dev_t *dev, uint8_t x, uint8_t y, double num, uint8_t int_len, uint8_t fra_len, uint8_t size);
static int8_t __oled_show_chinese(oled_dev_t *dev, uint8_t x, uint8_t y, char *chinese);
static int8_t __oled_printf(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t size, char *format, ...);
static int8_t __oled_draw_point(oled_dev_t *dev, uint8_t x, uint8_t y);
static uint8_t __oled_get_point(oled_dev_t *dev, uint8_t x, uint8_t y);
static int8_t __oled_draw_line(oled_dev_t *dev, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1);
static int8_t __oled_draw_rectangle(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint8_t is_filled);
static int8_t __oled_draw_triangle(oled_dev_t *dev, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t is_filled);
static int8_t __oled_draw_circle(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t radius, uint8_t is_filled);
static int8_t __oled_draw_ellipse(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t a, uint8_t b, uint8_t is_filled);
static int8_t __oled_draw_arc(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t radius, int16_t start_angle, int16_t end_angle, uint8_t is_filled);
static int8_t __oled_deinit(oled_dev_t *dev);
...
static int8_t __oled_write_command(oled_dev_t *dev, uint8_t command)
{
	...
	priv_data->i2c.recv_ack(&priv_data->i2c);
	priv_data->i2c.send_byte(&priv_data->i2c, 0x00);		// 控制字节，给0x00，表示即将写命令
	priv_data->i2c.recv_ack(&priv_data->i2c);
	priv_data->i2c.send_byte(&priv_data->i2c, command);	// 写入指定的命令
	priv_data->i2c.recv_ack(&priv_data->i2c);
 ...
 * @return	0, 表示成功, 其他值表示失败
 ******************************************************************************/
static int8_t __oled_set_cursor(oled_dev_t *dev, uint8_t page, uint8_t x)
{	
	if (!dev || !dev->init_flag)
		return -1;
...
static int8_t __oled_update(oled_dev_t *dev)
{
	...
	{
		/*设置光标位置为每一页的第一列*/
		__oled_set_cursor(dev, j, 0);
		/*连续写入128个数据，将显存数组的数据写入到OLED硬件*/
		__oled_write_data(dev, g_oled_diaplay_buf[priv_data->index][j], 128);
	}
...
static int8_t __oled_update_area(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height)
{
	...
	{
		/*设置光标位置为相关页的指定列*/
		__oled_set_cursor(dev, j, x);
		/*连续写入width个数据，将显存数组的数据写入到OLED硬件*/
		__oled_write_data(dev, &g_oled_diaplay_buf[priv_data->index][j][x], width);
	}
...
static int8_t __oled_show_image(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height, const uint8_t *image)
{
	...
	for (j = 0; j < (height - 1) / 8 + 1; j ++)
	{
		...
		for (i = 0; i < width; i ++)
		{
			...
			/*显示图像在当前页的内容*/
			g_oled_diaplay_buf[priv_data->index][y / 8 + j][x + i] |= image[j * width + i] << (y % 8);
			...
			/*显示图像在下一页的内容*/
			g_oled_diaplay_buf[priv_data->index][y / 8 + j + 1][x + i] |= image[j * width + i] >> (8 - y % 8);
		}
	}
...
{	
	...
	for (i = 0; i < len; i++)		//遍历数字的每一位							
	{
		...
		/*num / __oled_pow(10, len - i - 1) % 10 可以十进制提取数字的每一位*/
		/*+ '0' 可将数字转换为字符格式*/
...
{	
	...
	{
		/* 调用__oled_show_char函数，依次显示每个数字 */
		/* num / __oled_pow(2, len - i - 1) % 2 可以二进制提取数字的每一位 */
		/* + '0' 可将数字转换为字符格式 */
