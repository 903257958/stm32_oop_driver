...
#define XPT2046_DATA_ADDR   0x900000    // 校准数据在Flash中的存放地址
...
/* 函数声明 */
static void __xpt2046_write_correction_data(XPT2046Dev_t *pDev, float xfac, float yfac, float xoff, float yoff);
...
int xpt2046_init(XPT2046Dev_t *pDev)
{
    ...
    /* 检查是否已校准 */
    if (!flash.initFlag)    // Flash未初始化，返回错误
        return -1;

    uint8_t flashData[20] = {0};    // 用于存放读取的校准数据
    
    flash.read_data(&flash, XPT2046_DATA_ADDR, flashData, 20);       // 读取数据                  

    if ((flashData[0] == 'O') && (flashData[1] == 'K'))
    {
        /* 存有已校准的数据 */
        memcpy(&pPrivData->xfac, flashData + 2, 4);
        memcpy(&pPrivData->yfac, flashData + 6, 4);
        memcpy(&pPrivData->xoff, flashData + 10, 4);
        memcpy(&pPrivData->yoff, flashData + 14, 4);
    }
    else
    {
        /* 预先写入校正数据，以减少生产时手动校正操作 */
        __xpt2046_write_correction_data(pDev, 0.0647, 0.0889, -11.8483, -11.8666);
        __xpt2046_delay_us(10000);                  // 稍作延时
...
static void __xpt2046_write_correction_data(XPT2046Dev_t *pDev, float xfac, float yfac, float xoff, float yoff)
{
    ...
    flash.sector_erase(&flash, XPT2046_DATA_ADDR);   // 扇区擦除
    flash.write_data(&flash, XPT2046_DATA_ADDR + 0, (uint8_t *)&flag, 2);
    flash.write_data(&flash, XPT2046_DATA_ADDR + 2, (uint8_t *)&xfac, 4);
    flash.write_data(&flash, XPT2046_DATA_ADDR + 6, (uint8_t *)&yfac, 4);
    flash.write_data(&flash, XPT2046_DATA_ADDR + 10, (uint8_t *)&xoff, 4);
    flash.write_data(&flash, XPT2046_DATA_ADDR + 14, (uint8_t *)&yoff, 4);
}
...
static int  __xpt2046_recalibration(XPT2046Dev_t *pDev)
{
    ...
    /* 存入Flash中 */
    __xpt2046_write_correction_data(pDev, xfac, yfac, xoff, yoff);
