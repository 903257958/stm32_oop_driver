...
/* LCD数据端口与引脚数组 */
LCD_GPIO_Port gLCDDataPorts[] = {	GPIOD, GPIOD, GPIOD, GPIOD, 
...
typedef struct {
	...
	uint32_t		BLPin;		// 背光引脚
	LCD_GPIO_Port	CSPort;		// 片选端口
	uint32_t		CSPin;		// 片选引脚
	LCD_GPIO_Port	RSPort;		// 1数据/0命令端口
	uint32_t		RSPin;		// 1数据/0命令引脚
	LCD_GPIO_Port	WRPort;		// 写信号端口
...
static uint16_t __lcd_read_point(LCDDev_t *pDev, uint16_t x, uint16_t y);
...
static void __lcd_draw_circle(LCDDev_t *pDev, uint16_t x, uint16_t y, uint16_t r, uint16_t color);
...
int lcd_init(LCDDev_t *pDev)
{
	...
	pPrivData->dir = LCD_DIRECTION;
	pPrivData->wramcmd = 0X2C;
	...
	pPrivData->id = __lcd_read_data();
	pPrivData->id = __lcd_read_data();	// 0x0000
	pPrivData->id = __lcd_read_data();	// 0x0093
	pPrivData->id <<= 8;				// 0x9300
	pPrivData->id |= __lcd_read_data();	// 0x9341

	/* 设置WR时序为最快 */
	if (pPrivData->id == 0x9341)
	{
		/* 重新配置写时序控制寄存器的时序 */  	 							    
		FSMC_Bank1E->BWTR[6] &= ~(0XF << 0);	// 地址建立时间(ADDSET)清零 	 
		FSMC_Bank1E->BWTR[6] &= ~(0XF << 8);	// 数据保存时间清零
		FSMC_Bank1E->BWTR[6] |= 3 << 0;			// 地址建立时间(ADDSET)为3个HCLK = 18ns  	 
        if (pPrivData->id == 0X7789)
        {
	...
	/* 设置列地址的x坐标的EC初始值 */
	__lcd_write_cmd(pPrivData->setxcmd);
	__lcd_write_data(0);									// SC高8位 = 0
	__lcd_write_data(0);									// SC低8位 = 0
	__lcd_write_data((pDev->width - 1) >> 8);			// EC高8位 = 宽度-1的高8位
	__lcd_write_data((pDev->width - 1) & 0xFF);		// EC低8位 = 宽度-1的低8位

	/* 设置页地址的y坐标的EP初始值 */
	__lcd_write_cmd(pPrivData->setycmd);
	__lcd_write_data(0);									// SP高8位 = 0
	__lcd_write_data(0);									// SP低8位 = 0
	__lcd_write_data((pDev->height - 1) >> 8);			// EP高8位 = 高度-1的高8位
	__lcd_write_data((pDev->height - 1) & 0xFF);		// EP低8位 = 高度-1的低8位

	/* 设置屏幕方向 */
	if (pPrivData->dir == 0)	// 竖屏
    {
        pDev->width = 240;
        pDev->height = 320;
    }
	...
	pDev->read_point = __lcd_read_point;
 ...
 * @return	无
 ******************************************************************************/
static void __lcd_write_ram_prepare(LCDDev_t *pDev)
{
	LCDPrivData_t *pPrivData = (LCDPrivData_t *)pDev->pPrivData;
	
	LCD->LCD_REG = pPrivData->wramcmd;
}
 ...
 * @return	无
 ******************************************************************************/
static void __lcd_set_cursor(LCDDev_t *pDev, uint16_t x, uint16_t y)
{
	LCDPrivData_t *pPrivData = (LCDPrivData_t *)pDev->pPrivData;
...
{
	uint32_t index = 0;
    uint32_t totalpoint = pDev->width;
    totalpoint *= pDev->height;    	// 得到总点数

	__lcd_set_cursor(pDev, 0x0000, 0x0000);	// 设置光标位置
    __lcd_write_ram_prepare(pDev);			// 准备写GRAM
...
static void __lcd_fill(LCDDev_t *pDev, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t color)
{
    ...
    for (i = y; i <= y_end; i++)
    {
        __lcd_set_cursor(pDev, x, i);	// 设置光标位置
...
static void __lcd_show_char(LCDDev_t *pDev, uint16_t x, uint16_t y, uint8_t chr, uint16_t fc, uint16_t bc, uint8_t size, uint8_t mode)
{
	...
	{   
		...
		for(j = 0; j < 8; j++)
		{			    
			if(temp & 0x80)__lcd_draw_point(pDev, x, y, fc);
			else if(mode == 0)__lcd_draw_point(pDev, x, y, bc);
			temp <<= 1;
			y++;
...
static void __lcd_show_image(LCDDev_t *pDev, uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint8_t pic[])
{
	for (uint16_t i = 0; i < height; i++)           // 一行一行地显示
    {
        ...
        {
            LCD->LCD_RAM = pic[1] << 8 | *pic;  	// 写入16位颜色数据
            pic += 2;                             	// 数据指针向后移两字节
        }
    }
}
...
static void __lcd_draw_point(LCDDev_t *pDev, uint16_t x, uint16_t y, uint16_t color)
{
	__lcd_set_cursor(pDev, x, y);	// 设置光标位置
	__lcd_write_ram_prepare(pDev);	// 准备写GRAM
	LCD->LCD_RAM = color;			// 写入该点颜色
}
