...
#define	__spi_config_clock_enable(spix)		{	if(spix == SPI0)		{rcu_periph_clock_enable(RCU_SPI0);} \
												else if(spix == SPI1)	{rcu_periph_clock_enable(RCU_SPI1);} \
												else if(spix == SPI2)	{rcu_periph_clock_enable(RCU_SPI2);} \
											}

#define	__spi_config_gpio_clock_enable(port)	{	if(port == GPIOA)		{rcu_periph_clock_enable(RCU_GPIOA);} \
...
static int8_t __spi_deinit(SPIDev_t *dev);
