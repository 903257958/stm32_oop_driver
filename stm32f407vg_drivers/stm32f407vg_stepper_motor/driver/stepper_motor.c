...
#define	__stepper_motor_io_clock_enable(port)	{	if (port == GPIOA)		{RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);} \
...
#define	__stepper_motor_io_clock_enable(port)	{	if (port == GPIOA)		{RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA, ENABLE);} \
...
/* 函数声明 */
static void __stepper_motor_control(stepper_motor_dev_t *dev, int cnt, uint16_t time_ms);
static void __stepper_motor_disable(stepper_motor_dev_t *dev);
static int8_t __stepper_motor_deinit(stepper_motor_dev_t *dev);

/******************************************************************************
 * @brief	初始化步进电机
 * @param	dev	:	stepper_motor_dev_t 结构体指针
...
int8_t stepper_motor_init(stepper_motor_dev_t *dev)
{
	...
	/* 配置时钟与GPIO */
	__stepper_motor_io_clock_enable(dev->config.in1_port);
	__stepper_motor_io_clock_enable(dev->config.in2_port);
	__stepper_motor_io_clock_enable(dev->config.in3_port);
	__stepper_motor_io_clock_enable(dev->config.in4_port);
 ...
 * @return	无
 ******************************************************************************/
static void __stepper_motor_set_pins(stepper_motor_dev_t *dev, int index)
{
...
static void __stepper_motor_control(stepper_motor_dev_t *dev, int cnt, uint16_t time_ms)
{
	...
	if (cnt >= 0)
	{
		...
		for (step = 0; step < cnt; step++)
		{
			...
			STEPPER_MOTOR_DELAY_MS(time_ms);
