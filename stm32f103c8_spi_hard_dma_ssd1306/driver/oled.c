...
#define	__oled_dma_clock_enable(spix)	{	if (spix == SPI1)		{RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA1, ENABLE);} \
...
#define	__oled_io_clock_enable(port)	{	if (port == GPIOA)		{RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA, ENABLE);} \
...
#define	__oled_dma_clock_enable(spix)	{	if (spix == SPI1)		{RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA2, ENABLE);} \
...
/* 通信协议 */
static void __oled_res_write(oled_dev_t *dev, uint8_t bit_val);
static void __oled_dc_write(oled_dev_t *dev, uint8_t bit_val);
static int __oled_write_command(oled_dev_t *dev, uint8_t command);
static int __oled_write_data(oled_dev_t *dev, uint8_t *data, uint8_t count);
static int __oled_write_data_dma(oled_dev_t *dev, uint8_t *data, uint16_t count);

/* 功能函数 */
static int __oled_update(oled_dev_t *dev);
static int __oled_update_area(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height);
static int __oled_clear(oled_dev_t *dev);
static int __oled_clear_area(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height);
static int __oled_reverse(oled_dev_t *dev);
static int __oled_reverse_area(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height);
static int __oled_show_image(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height, const uint8_t *image);
static int __oled_show_char(oled_dev_t *dev, uint8_t x, uint8_t y, char chr, uint8_t size);
static int __oled_show_string(oled_dev_t *dev, uint8_t x, uint8_t y, char *string, uint8_t size);
static int __oled_show_num(oled_dev_t *dev, uint8_t x, uint8_t y, uint32_t num, uint8_t len, uint8_t size);
static int __oled_show_signed_num(oled_dev_t *dev, uint8_t x, uint8_t y, int32_t num, uint8_t len, uint8_t size);
static int __oled_show_hex_num(oled_dev_t *dev, uint8_t x, uint8_t y, uint32_t num, uint8_t len, uint8_t size);
static int __oled_show_bin_num(oled_dev_t *dev, uint8_t x, uint8_t y, uint32_t num, uint8_t len, uint8_t size);
static int __oled_show_float_num(oled_dev_t *dev, uint8_t x, uint8_t y, double num, uint8_t int_len, uint8_t fra_len, uint8_t size);
static int __oled_show_chinese(oled_dev_t *dev, uint8_t x, uint8_t y, char *chinese);
static int __oled_printf(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t size, char *format, ...);
static int __oled_draw_point(oled_dev_t *dev, uint8_t x, uint8_t y);
static uint8_t __oled_get_point(oled_dev_t *dev, uint8_t x, uint8_t y);
static int __oled_draw_line(oled_dev_t *dev, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1);
static int __oled_draw_rectangle(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint8_t is_filled);
static int __oled_draw_triangle(oled_dev_t *dev, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t is_filled);
static int __oled_draw_circle(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t radius, uint8_t is_filled);
static int __oled_draw_ellipse(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t a, uint8_t b, uint8_t is_filled);
static int __oled_draw_arc(oled_dev_t *dev, uint8_t x, uint8_t y, uint8_t radius, int16_t start_angle, int16_t end_angle, uint8_t is_filled);
static int __oled_deinit(oled_dev_t *dev);

/******************************************************************************
 * @brief	初始化OLED
 * @param	dev		:	oled_dev_t 结构体指针
...
int oled_init(oled_dev_t *dev)
{
	...
	/* 配置时钟与GPIO */
	__oled_io_clock_enable(dev->config.res_port);
	__oled_io_clock_enable(dev->config.dc_port);
...
static int __oled_write_data_dma(oled_dev_t *dev, uint8_t *data, uint16_t count)
{
	...
	__oled_dma_clock_enable(dev->config.spix);		// 开启DMA时钟
 ...
 * @return	0, 表示成功, 其他值表示失败
 ******************************************************************************/
static int OLED_SetCursor(oled_dev_t *dev, uint8_t page, uint8_t x)
{	
	if (!dev || !dev->init_flag)
		return -1;
 ...
 * @return	x^y
 ******************************************************************************/
static uint32_t __oled_pow(uint32_t x, uint32_t y)
{
 ...
 ******************************************************************************/
static uint8_t __oled_pnpoly(uint8_t nvert, int16_t *vertx, int16_t *verty, int16_t testx, int16_t testy)
{
	int16_t i, j, c = 0;
 ...
 ******************************************************************************/
static uint8_t __oled_is_in_angle(int16_t x, int16_t y, int16_t start_angle, int16_t end_angle)
{
	int16_t point_angle;
...
{	
	...
	for (i = 0; i < len; i++)		// 遍历数字的每一位							
	{
		...
		/* Number / __oled_pow(10, len - i - 1) % 10 可以十进制提取数字的每一位 */
		/* + '0' 可将数字转换为字符格式 */
		__oled_show_char(dev, x + i * size, y, num / __oled_pow(10, len - i - 1) % 10 + '0', size);
	}
...
{	
	...
	{
		/* 调用__oled_show_char函数，依次显示每个数字 */
		/* num1 / __oled_pow(10, len - i - 1) % 10 可以十进制提取数字的每一位 */
		/* + '0' 可将数字转换为字符格式 */
		__oled_show_char(dev, x + (i + 1) * size, y, num1 / __oled_pow(10, len - i - 1) % 10 + '0', size);
	}
...
{	
	...
	{
		/* 以十六进制提取数字的每一位 */
		single_num = num / __oled_pow(16, len - i - 1) % 16;
...
{	
	...
	{
		/* 调用__oled_show_char函数，依次显示每个数字 */
		/* Number / __oled_pow(2, len - i - 1) % 2 可以二进制提取数字的每一位 */
		/* + '0' 可将数字转换为字符格式 */
		__oled_show_char(dev, x + i * size, y, num / __oled_pow(2, len - i - 1) % 2 + '0', size);
	}
 ...
 * @param	int_len		:	指定数字的整数位长度，范围：0~10
 * @param	fra_len		:	指定数字的小数位长度，范围：0~9，小数进行四舍五入显示
 * @param	size		:	指定字体大小
 * @return	0, 表示成功, 其他值表示失败
 ******************************************************************************/
static int __oled_show_float_num(oled_dev_t *dev, uint8_t x, uint8_t y, double num, uint8_t int_len, uint8_t fra_len, uint8_t size)
{	
	...
	pow_num = __oled_pow(10, fra_len);		// 根据指定小数的位数，确定乘数
	fra_num = round(num * pow_num);		// 将小数乘到整数，同时四舍五入，避免显示误差
	int_num += fra_num / pow_num;				// 若四舍五入造成了进位，则需要再加给整数
	
	/* 显示整数部分 */
	__oled_show_num(dev, x + size, y, int_num, int_len, size);
	
	/* 显示小数点 */
	__oled_show_char(dev, x + (int_len + 1) * size, y, '.', size);
	
	/* 显示小数部分 */
	__oled_show_num(dev, x + (int_len + 2) * size, y, fra_num, fra_len, size);
...
{	
	...
	else					// 指定三角形填充
	{
		...
		for (i = minx; i <= maxx; i ++)
		{
			...
			for (j = miny; j <= maxy; j ++)
			{
				/* 调用__oled_pnpoly，判断指定点是否在指定三角形之中 */
				/* 如果在，则画点，如果不在，则不做处理 */
				if (__oled_pnpoly(3, vx, vy, i, j)) {__oled_draw_point(dev, i, j);}
			}
		}
	}
...
{	
	...
	if (__oled_is_in_angle(x1, y1, start_angle, end_angle))	{__oled_draw_point(dev, x + x1, y + y1);}
	if (__oled_is_in_angle(-x1, -y1, start_angle, end_angle)) {__oled_draw_point(dev, x - x1, y - y1);}
	if (__oled_is_in_angle(y1, x1, start_angle, end_angle)) {__oled_draw_point(dev, x + y1, y + x1);}
	if (__oled_is_in_angle(-y1, -x1, start_angle, end_angle)) {__oled_draw_point(dev, x - y1, y - x1);}
	...
	if (is_filled)	// 指定圆弧填充
	{
		...
		for (j = -y1; j < y1; j ++)
		{
			...
			if (__oled_is_in_angle(0, j, start_angle, end_angle)) {__oled_draw_point(dev, x, y + j);}
		}
	}
	...
	while (x1 < y1)		// 遍历X轴的每个点
	{
		...
		if (is_filled)	// 指定圆弧填充
		{
			...
			{
				/* 在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理 */
				if (__oled_is_in_angle(x1, j, start_angle, end_angle)) {__oled_draw_point(dev, x + x1, y + j);}
				if (__oled_is_in_angle(-x1, j, start_angle, end_angle)) {__oled_draw_point(dev, x - x1, y + j);}
			}
			...
			{
				/* 在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理 */
				if (__oled_is_in_angle(-y1, j, start_angle, end_angle)) {__oled_draw_point(dev, x - y1, y + j);}
				if (__oled_is_in_angle(y1, j, start_angle, end_angle)) {__oled_draw_point(dev, x + y1, y + j);}
			}
		}
	}
