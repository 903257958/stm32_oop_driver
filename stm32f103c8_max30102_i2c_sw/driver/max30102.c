...
#define	__max30102_config_gpio_clock_enable(port)	{	if(port == GPIOA)		{RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);} \
...
/* 函数声明 */
static int8_t __max30102_software_init(MAX30102Dev_t *dev);
static int8_t __max30102_read_fifo(MAX30102Dev_t *dev, uint32_t *red_led_data, uint32_t *ir_led_data);
static int8_t __max30102_get_data(MAX30102Dev_t *dev);
static int8_t __max30102_deinit(MAX30102Dev_t *dev);
 ...
 ******************************************************************************/
int8_t max30102_init(MAX30102Dev_t *dev)
{
    if (!dev)
		return -1;
...
static int8_t __max30102_get_data(MAX30102Dev_t *dev)
{
	...
	if (dis_hr > 0 && dis_hr < 140)
		dev->heart_rate = dis_hr - 20;

	if (dis_spo2 > 0 && dis_hr < 100)
		dev->blood_oxygen = dis_spo2;
