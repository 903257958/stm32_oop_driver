...
static int __oled_show_char(OLEDDev_t *dev, uint8_t x, uint8_t y, char Char, uint8_t fontSize);
static int __oled_show_string(OLEDDev_t *dev, uint8_t x, uint8_t y, char *string, uint8_t fontSize);
static int __oled_show_num(OLEDDev_t *dev, uint8_t x, uint8_t y, uint32_t number, uint8_t length, uint8_t fontSize);
static int __oled_show_signed_num(OLEDDev_t *dev, uint8_t x, uint8_t y, int32_t number, uint8_t length, uint8_t fontSize);
static int __oled_show_hex_num(OLEDDev_t *dev, uint8_t x, uint8_t y, uint32_t number, uint8_t length, uint8_t fontSize);
static int __oled_show_bin_num(OLEDDev_t *dev, uint8_t x, uint8_t y, uint32_t number, uint8_t length, uint8_t fontSize);
static int __oled_show_float_num(OLEDDev_t *dev, uint8_t x, uint8_t y, double number, uint8_t intLength, uint8_t fraLength, uint8_t fontSize);
static int __oled_show_chinese(OLEDDev_t *dev, uint8_t x, uint8_t y, char *Chinese);
static int __oled_printf(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t fontSize, char *format, ...);
...
static int __oled_draw_rectangle(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint8_t isFilled);
static int __oled_draw_triangle(OLEDDev_t *dev, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t IsFilled);
static int __oled_draw_circle(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t radius, uint8_t IsFilled);
static int __oled_draw_ellipse(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t a, uint8_t b, uint8_t isFilled);
static int __oled_draw_arc(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t radius, int16_t startAngle, int16_t endAngle, uint8_t isFilled);
...
static int __oled_update(OLEDDev_t *dev)
{
	...
	for (j = 0; j < 8; j ++)
	{
		...
		__oled_write_data(dev, gOLEDDisplayBuf[priv_data->index][j], 128);
	}
...
static int __oled_update_area(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height)
{
	...
	for (j = y / 8; j < (y + height - 1) / 8 + 1; j ++)
	{
		...
		__oled_write_data(dev, &gOLEDDisplayBuf[priv_data->index][j][x], width);
	}
...
static int __oled_clear(OLEDDev_t *dev)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		...
		{
			gOLEDDisplayBuf[priv_data->index][j][i] = 0x00;	//将显存数组数据全部清零
		}
	}
...
static int __oled_clear_area(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height)
{
	...
	for (j = y; j < y + height; j ++)		//遍历指定页
	{
		...
		{
			gOLEDDisplayBuf[priv_data->index][j / 8][i] &= ~(0x01 << (j % 8));	//将显存数组指定数据清零
		}
	}
...
static int __oled_reverse(OLEDDev_t *dev)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		for (i = 0; i < 128; i ++)			//遍历128列
		{
			gOLEDDisplayBuf[priv_data->index][j][i] ^= 0xFF;	//将显存数组数据全部取反
		}
	}
...
static int __oled_reverse_area(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height)
{
	...
	for (j = y; j < y + height; j ++)		//遍历指定页
	{
		for (i = x; i < x + width; i ++)	//遍历指定列
		{
			gOLEDDisplayBuf[priv_data->index][j / 8][i] ^= 0x01 << (j % 8);	//将显存数组指定数据取反
		}
	}
...
static int __oled_show_image(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height, const uint8_t *image)
{
	...
	for (j = 0; j < (height - 1) / 8 + 1; j ++)
	{
		...
		for (i = 0; i < width; i ++)
		{
			...
			/*显示图像在当前页的内容*/
			gOLEDDisplayBuf[priv_data->index][y / 8 + j][x + i] |= image[j * width + i] << (y % 8);
			...
			/*显示图像在下一页的内容*/
			gOLEDDisplayBuf[priv_data->index][y / 8 + j + 1][x + i] |= image[j * width + i] >> (8 - y % 8);
		}
	}
...
static int __oled_draw_point(OLEDDev_t *dev, uint8_t x, uint8_t y)
{
	...
	/*将显存数组指定位置的一个Bit数据置1*/
	gOLEDDisplayBuf[priv_data->index][y / 8][x] |= 0x01 << (y % 8);
...
static uint8_t __oled_get_point(OLEDDev_t *dev, uint8_t x, uint8_t y)
{
	...
	/*判断指定位置的数据*/
	if (gOLEDDisplayBuf[priv_data->index][y / 8][x] & 0x01 << (y % 8))
	{
		return 1;	//为1，返回1
	}
	return 0;		//否则，返回0
