...
#define UART2_TX_SIZE   2048    // UART2发送缓冲区长度
#define UART2_RX_SIZE   2048    // UART2接收缓冲区长度
#define UART2_RX_MAX    256     // UART2单次接收最大长度
...
extern uint8_t uart2_tx_data_buf[UART2_TX_SIZE];	// UART2发送数据缓冲区
extern uint8_t uart2_rx_data_buf[UART2_RX_SIZE];	// UART2接收数据缓冲区
...
extern UARTRXControlBlock_t uart2_rx_control_block;	// UART2接收控制块结构体

/* 函数声明 */
void uart0_init(uint32_t baud);
void uart0_printf(char *format, ...);
void uart0_recv_data(void);
char *__uart0_recv(void);
