...
uint8_t uart2_tx_data_buf[UART2_TX_SIZE];		// UART2发送数据缓冲区
uint8_t uart2_rx_data_buf[UART2_RX_SIZE];		// UART2接收数据缓冲区

UARTRXControlBlock_t uart0_rx_control_block;	// UART0接收控制块结构体
UARTRXControlBlock_t uart2_rx_control_block;	// UART2接收控制块结构体

/* 函数声明 */
static void uart0_dma_init(void);
static void uart2_dma_init(void);
static void uart0_control_block_init(void);
static void uart2_control_block_init(void);
...
static void uart2_dma_init(void)
{
	...
	dma_parameter_struct dma_init_struct;
	dma_init_struct.periph_addr = USART2 + 4;						// 外设基地址，数据寄存器偏移0x04
	dma_init_struct.periph_width = DMA_PERIPHERAL_WIDTH_8BIT;		// 外设数据宽度
	dma_init_struct.memory_addr = (uint32_t)uart2_rx_data_buf;		// 内存基地址
	dma_init_struct.memory_width = DMA_MEMORY_WIDTH_8BIT;			// 内存数据宽度
	dma_init_struct.number = UART2_RX_MAX + 1;						// 只有空闲中断才能判断接收完成，不会产生DMA完成中断
	dma_init_struct.priority = DMA_PRIORITY_HIGH;					// 优先级
	dma_init_struct.periph_inc = DMA_PERIPH_INCREASE_DISABLE;		// 外设不递增
	dma_init_struct.memory_inc = DMA_MEMORY_INCREASE_ENABLE;		// 内存递增
	dma_init_struct.direction = DMA_PERIPHERAL_TO_MEMORY;			// 从外设到内存
...
{
	/* 初始化索引指针指向索引数组的第0位 */
	uart2_rx_control_block.index_in = &uart2_rx_control_block.index_buf[0];
	uart2_rx_control_block.index_out = &uart2_rx_control_block.index_buf[0];

	/* 标记索引数组的end */
	uart2_rx_control_block.index_end = &uart2_rx_control_block.index_buf[INDEX_BUF_NUM - 1];

	/* 标记第一段数据的start为接收数据的第0位 */
	uart2_rx_control_block.index_in->start = &uart2_rx_data_buf[0];
}
...
void uart2_printf(char *format, ...)
{
	...
	va_start(list_data, format);
	vsprintf((char *)uart2_tx_data_buf, format, list_data);
	va_end(list_data);

	for (i = 0; i < strlen((const char *)uart2_tx_data_buf); i++)
	{
		while (usart_flag_get(USART2, USART_FLAG_TBE) != 1);
		usart_data_transmit(USART2, uart2_tx_data_buf[i]);
	}
...
void USART2_IRQHandler(void)
{
	if (usart_interrupt_flag_get(USART2, USART_INT_FLAG_IDLE) != 0)
	{
		...
		/* DMA传输数据总量 - DMA数据剩余量 = 已接收数据量 */
		uart2_rx_control_block.data_cnt += ((UART2_RX_MAX + 1) - dma_transfer_number_get(DMA0, DMA_CH2));

		/* 标记这一段数据的end */
		uart2_rx_control_block.index_in->end = &uart2_rx_data_buf[uart2_rx_control_block.data_cnt - 1];

		/* 接收数据后in指针向后移动一位 */
		uart2_rx_control_block.index_in++;
		if (uart2_rx_control_block.index_in == uart2_rx_control_block.index_end)
		{
			uart2_rx_control_block.index_in = &uart2_rx_control_block.index_buf[0];
		}

		/* 判断数据缓冲区剩余大小 */
		if (UART2_RX_SIZE - uart2_rx_control_block.data_cnt >= UART2_RX_MAX)
		{
			/* 剩余大小足够再接收一次数据，标记下一段数据的start */
			uart2_rx_control_block.index_in->start = &uart2_rx_data_buf[uart2_rx_control_block.data_cnt];
		}
		else
		{
			/* 剩余大小不够再接收一次数据，回卷 */
			uart2_rx_control_block.index_in->start = &uart2_rx_data_buf[0];
			uart2_rx_control_block.data_cnt = 0;
		}

		/* DMA准备下一次接收 */
		dma_channel_disable(DMA0, DMA_CH2);
		dma_transfer_number_config(DMA0, DMA_CH2, UART2_RX_MAX + 1);
		dma_memory_address_config(DMA0, DMA_CH2, (uint32_t)uart2_rx_control_block.index_in->start);
		dma_channel_enable(DMA0, DMA_CH2);
	}
}
