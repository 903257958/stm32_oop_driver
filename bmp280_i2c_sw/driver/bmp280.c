...
static int __bmp280_pressure_compensate(BMP280Dev_t *dev, int32_t *pressure_raw);
static int __bmp280_altitude_calculate(BMP280Dev_t *dev);
...
static int __bmp280_altitude_calculate(BMP280Dev_t *dev)
{
	...
	const float P0 = 1013.250f;   						// 标准海平面气压（单位：hPa）
	const float T_K = dev->data.temperature + 273.15f;	// 转换为开尔文温度

	/* Hypsometric公式 */
	float ratio =  dev->data.pressure / P0;
	dev->data.altitude = (powf(ratio, 0.1903f) - 1.0f) * T_K / 0.0065f;
...
static int __bmp280_get_data(BMP280Dev_t *dev)
{
	...
	/* 海拔计算 */
	__bmp280_altitude_calculate(dev);
