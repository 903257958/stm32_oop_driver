...
/* 通信协议 */
static int8_t __oled_write_command(OLEDDev_t *dev, uint8_t command);
static int8_t __oled_write_data(OLEDDev_t *dev, uint8_t *data, uint8_t count);

/* 功能函数 */
static int8_t __oled_update(OLEDDev_t *dev);
static int8_t __oled_update_area(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height);
static int8_t __oled_clear(OLEDDev_t *dev);
static int8_t __oled_clear_area(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height);
static int8_t __oled_reverse(OLEDDev_t *dev);
static int8_t __oled_reverse_area(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height);
static int8_t __oled_show_image(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height, const uint8_t *image);
static int8_t __oled_show_char(OLEDDev_t *dev, uint8_t x, uint8_t y, char Char, uint8_t fontSize);
static int8_t __oled_show_string(OLEDDev_t *dev, uint8_t x, uint8_t y, char *string, uint8_t fontSize);
static int8_t __oled_show_num(OLEDDev_t *dev, uint8_t x, uint8_t y, uint32_t number, uint8_t length, uint8_t fontSize);
static int8_t __oled_show_signed_num(OLEDDev_t *dev, uint8_t x, uint8_t y, int32_t number, uint8_t length, uint8_t fontSize);
static int8_t __oled_show_hex_num(OLEDDev_t *dev, uint8_t x, uint8_t y, uint32_t number, uint8_t length, uint8_t fontSize);
static int8_t __oled_show_bin_num(OLEDDev_t *dev, uint8_t x, uint8_t y, uint32_t number, uint8_t length, uint8_t fontSize);
static int8_t __oled_show_float_num(OLEDDev_t *dev, uint8_t x, uint8_t y, double number, uint8_t intLength, uint8_t fraLength, uint8_t fontSize);
static int8_t __oled_show_chinese(OLEDDev_t *dev, uint8_t x, uint8_t y, char *Chinese);
static int8_t __oled_printf(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t fontSize, char *format, ...);
static int8_t __oled_draw_point(OLEDDev_t *dev, uint8_t x, uint8_t y);
static uint8_t __oled_get_point(OLEDDev_t *dev, uint8_t x, uint8_t y);
static int8_t __oled_draw_line(OLEDDev_t *dev, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1);
static int8_t __oled_draw_rectangle(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint8_t isFilled);
static int8_t __oled_draw_triangle(OLEDDev_t *dev, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t IsFilled);
static int8_t __oled_draw_circle(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t radius, uint8_t IsFilled);
static int8_t __oled_draw_ellipse(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t a, uint8_t b, uint8_t isFilled);
static int8_t __oled_draw_arc(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t radius, int16_t startAngle, int16_t endAngle, uint8_t isFilled);
static int8_t __oled_deinit(OLEDDev_t *dev);
 ...
 * @return	0, 表示成功, 其他值表示失败
 ******************************************************************************/
static int8_t OLED_SetCursor(OLEDDev_t *dev, uint8_t page, uint8_t x)
{	
	if (!dev || !dev->init_flag)
		return -1;
...
{	
	...
	if (fontSize == OLED_8X16)		//字体为宽8像素，高16像素
	{
		...
		__oled_show_image(dev, x, y, 8, 16, OLED_F8x16[Char - ' ']);
	}
	else if(fontSize == OLED_6X8)	//字体为宽6像素，高8像素
	{
		/*将ASCII字模库OLED_F6x8的指定数据以6*8的图像格式显示*/
		__oled_show_image(dev, x, y, 6, 8, OLED_F6x8[Char - ' ']);
	}
...
{	
	...
	for (i = 0; string[i] != '\0'; i++)		//遍历字符串的每个字符
	{
		...
		__oled_show_char(dev, x + i * fontSize, y, string[i], fontSize);
	}
...
{	
	...
	for (i = 0; i < length; i++)		//遍历数字的每一位							
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		__oled_show_char(dev, x + i * fontSize, y, number / OLED_Pow(10, length - i - 1) % 10 + '0', fontSize);
	}
...
static int8_t __oled_show_signed_num(OLEDDev_t *dev, uint8_t x, uint8_t y, int32_t number, uint8_t length, uint8_t fontSize)
{
	...
	if (number >= 0)								//数字大于等于0
	{
		__oled_show_char(dev, x, y, '+', fontSize);	//显示+号
	...
	for (i = 0; i < length; i++)					//遍历数字的每一位								
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		__oled_show_char(dev, x + (i + 1) * fontSize, y, Number1 / OLED_Pow(10, length - i - 1) % 10 + '0', fontSize);
	}
...
{	
	...
	for (i = 0; i < length; i++)		//遍历数字的每一位
	{
		...
		if (SingleNumber < 10)			//单个数字小于10
		{
			...
			/*+ '0' 可将数字转换为字符格式*/
			__oled_show_char(dev, x + i * fontSize, y, SingleNumber + '0', fontSize);
		}
		else							//单个数字大于10
		{
			/*调用__oled_show_char函数，显示此数字*/
			/*+ 'A' 可将数字转换为从A开始的十六进制字符*/
			__oled_show_char(dev, x + i * fontSize, y, SingleNumber - 10 + 'A', fontSize);
		}
	}
...
{	
	...
	for (i = 0; i < length; i++)		//遍历数字的每一位	
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		__oled_show_char(dev, x + i * fontSize, y, number / OLED_Pow(2, length - i - 1) % 2 + '0', fontSize);
	}
...
{	
	...
	/*显示整数部分*/
	__oled_show_num(dev, x + fontSize, y, IntNum, intLength, fontSize);
	
	/*显示小数点*/
	__oled_show_char(dev, x + (intLength + 1) * fontSize, y, '.', fontSize);
	
	/*显示小数部分*/
	__oled_show_num(dev, x + (intLength + 2) * fontSize, y, FraNum, fraLength, fontSize);
...
{	
	...
	for (i = 0; Chinese[i] != '\0'; i ++)		//遍历汉字串
	{
		...
		if (pChinese >= OLED_CHN_CHAR_WIDTH)
		{
			...
			__oled_show_image(dev, x + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 16, y, 16, 16, OLED_CF16x16[pIndex].Data);
		}
	}
...
{	
	...
	va_end(arg);							//结束变量arg
	__oled_show_string(dev, x, y, string, fontSize);//OLED显示字符数组（字符串）
...
static int8_t __oled_draw_arc(OLEDDev_t *dev, uint8_t x, uint8_t y, uint8_t radius, int16_t startAngle, int16_t endAngle, uint8_t isFilled)
{
	...
	if (OLED_IsInAngle(x1, y1, startAngle, endAngle))	{__oled_draw_point(dev, x + x1, y + y1);}
	if (OLED_IsInAngle(-x1, -y1, startAngle, endAngle)) {__oled_draw_point(dev, x - x1, y - y1);}
	if (OLED_IsInAngle(y1, x1, startAngle, endAngle)) {__oled_draw_point(dev, x + y1, y + x1);}
	if (OLED_IsInAngle(-y1, -x1, startAngle, endAngle)) {__oled_draw_point(dev, x - y1, y - x1);}
	...
	if (isFilled)	//指定圆弧填充
	{
		...
		for (j = -y1; j < y1; j ++)
		{
			...
			if (OLED_IsInAngle(0, j, startAngle, endAngle)) {__oled_draw_point(dev, x, y + j);}
		}
	}
	...
	while (x1 < y1)		//遍历X轴的每个点
	{
		...
		if (isFilled)	//指定圆弧填充
		{
			...
			{
				/*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
				if (OLED_IsInAngle(x1, j, startAngle, endAngle)) {__oled_draw_point(dev, x + x1, y + j);}
				if (OLED_IsInAngle(-x1, j, startAngle, endAngle)) {__oled_draw_point(dev, x - x1, y + j);}
			}
			...
			{
				/*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
				if (OLED_IsInAngle(-y1, j, startAngle, endAngle)) {__oled_draw_point(dev, x - y1, y + j);}
				if (OLED_IsInAngle(y1, j, startAngle, endAngle)) {__oled_draw_point(dev, x + y1, y + j);}
			}
		}
	}
